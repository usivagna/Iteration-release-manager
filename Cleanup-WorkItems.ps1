...
if (-not $useAzCli) {
    if ($null -eq $PAT -or $PAT.Length -eq 0) {
        ...
        if (-not [string]::IsNullOrEmpty($patEnvVar)) {
            ...
            Write-Host "  2. Pass PAT as SecureString: `$secPat = ConvertTo-SecureString 'your-pat' -AsPlainText -Force; .\Cleanup-WorkItems.ps1 -PAT `$secPat" -ForegroundColor Cyan
...
function Invoke-ADORestAPI {
    ...
    try {
        ...
        if ($_.ErrorDetails.Message) {
            ...
            Write-Host "Operation failed. Please check your permissions." -ForegroundColor Red
        }
        return $null
    }
}
...
try {
    ...
    if ($null -eq $permissionTest) {
        ...
        Write-Host "You may not have the required permissions in project '$ProjectName'" -ForegroundColor Yellow
    ...
    Write-Host "Please verify you have 'Work Items (Read & Write)' permission in project '$ProjectName'" -ForegroundColor Yellow
    exit 1
}
Write-Host ""
